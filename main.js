...
import { getFirestore, doc, getDoc, setDoc, updateDoc, onSnapshot, increment } 
...
onAuthStateChanged(auth,(user)=>{
  if(user){
    ...
    if(user.email === "gbx100k@gmail.com"){
      ...
      document.getElementById("contadorGlobal").style.display="none";
    }
    ...
    getDoc(ref).then(snap=>{
      if(!snap.exists()){
        setDoc(ref,{green:0,red:0});
      }
    });

    // Atualiza contador global em tempo real
    const refSnap = doc(db, "historico", "global");
    onSnapshot(refSnap,(docSnap)=>{
      if(docSnap.exists()){
        document.getElementById("contadorGlobal").innerText=
          "Global: "+docSnap.data().green+" Green | "+docSnap.data().red+" Red";
      }
    });
...
export function marcar(tipo){
  ...
  onAuthStateChanged(auth,(user)=>{
    if(user && user.email==="gbx100k@gmail.com"){
      updateDoc(doc(db,"historico","global"),(tipo==="GREEN")?{green:increment(1)}:{red:increment(1)});
    }
  });
}